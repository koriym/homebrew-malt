...
- Global pid registry: every service start records pids in `~/.malt/pids/` (override with `MALT_REGISTRY_DIR`). Both `~/.malt` and `~/.malt/pids` are forced to `0700`, and `registry_entries` skips a registry directory or entry file that is not a plain directory/file owned by the current uid and writable only by it. `malt kill` only SIGKILLs registry pids whose live command line still matches the identity recorded at start, so it can never kill Homebrew-managed services. Never reintroduce `pkill -f` on bare process names.
