...
`malt.json`をインフラストラクチャの`composer.json`と考えてください - これはプロジェクトのインフラストラクチャ依存関係の単一の信頼できる情報源として機能します。`composer install`がPHP依存関係を取り込むのと同様に、`malt install && malt create && malt start`はインフラストラクチャ依存関係をセットアップします。
...
既存のチームに参加する場合や2台目の開発マシンを設定する場合は、依存関係をインストールし、`malt/`ディレクトリがまだ無ければ生成してからサービスを開始します：
...
- `malt start` - 設定されたサービスを開始する
- `malt stop` - 現在のプロジェクトの設定済みサービスを停止する
- `malt kill` - malt が起動したサービスプロセスのみを、プロジェクトに関係なく強制終了する
- `malt env` - 環境変数を表示する
