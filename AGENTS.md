...
- Global pid registry: every service start records pids in `~/.malt/pids/` (override with `MALT_REGISTRY_DIR`). Both `~/.malt` and `~/.malt/pids` are forced to `0700`, and `registry_entries` skips a registry directory or entry file that is not a plain directory/file owned by the current uid and writable only by it. `HOMEBREW_PREFIX/var/malt/pids/`, where pids were recorded before the move, is still read under the same checks so `malt kill` reaches services an older malt started; it is never written to, so it drains itself. `malt kill` only SIGKILLs registry pids whose live command line still matches the identity recorded at start, so it can never kill Homebrew-managed services. Never reintroduce `pkill -f` on bare process names.
- Non-daemonizing services (mysqld_safe, php-fpm `-y`, redis-server, httpd) are spawned via `Process.spawn` with stdout/stderr redirected to files in `malt/logs/` (keeps `malt start | ...` pipes closable) and `pgroup: true`, so `malt kill` can SIGKILL the whole process group (supervisor plus forked workers) without enumerating children. Nginx (`daemon on`) and Memcached (`-d`) fork themselves and are launched with `system` instead.
